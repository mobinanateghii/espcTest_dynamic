...
mongoose.connect(db , {useUnifiedTopology: true , useNewUrlParser: true}).then(() => console.log('DB Connected!'))
    .catch(err => {
        ...
        espcApi.get('/paymentData' , (req , res)=>{
        ...
        espcApi.post('/paymentData/creatData' , (req , res)=>{
            let data = req.body;
        ...
        espcApi.delete('/paymentData/:id/delete' , (req , res)=>{
        ...
        espcApi.patch('/paymentData/:id/edit' , (req , res)=>{
            const data = req.body;
            PayData.findById(req.params.id , (err , record)=>{
               if(err) console.log(err);
               else if(!record) res.status(404).send("couldn't find data");
